...
// var swooshAudio = new Audio("./beep3.mp3");
...
function speakAlgorithm(algo) {
  const synth = window.speechSynthesis;
  const utterance = new SpeechSynthesisUtterance(`You choose ${algo} algorithm.`);
  synth.speak(utterance);
}

function speakArraySorted(algo) {
  const synth = window.speechSynthesis;
  const utterance = new SpeechSynthesisUtterance(`Sorting Done Using ${algo} algorithm.`);
  synth.speak(utterance);
  document.getElementById('status').innerHTML = `Sorting Done Using ${algo} algorithm.`;
...
const SortAlgo = {
  ...
  selectionSort() {
    ...
    async function sort(self) {
      ...
      isFound = true;
      // togglePlay();
      speakArraySorted("Selection Sort");
    }
...
function startSorting() {
  let algo = document.getElementById("get-algo").value;
  if (algo == "bubble-sort") {
    speakAlgorithm("bubble sort");
    const bubbleSortStarted = SortAlgo.bubbleSort.bind(SortAlgo);
    bubbleSortStarted();
  } else if (algo == "selection-sort") {
    speakAlgorithm("selection sort");
    const selectionSortStarted = SortAlgo.selectionSort.bind(SortAlgo);
    selectionSortStarted();
  } else if (algo == "merge-sort") {
    speakAlgorithm("Merge sort");
    const mergeSortStarted = SortAlgo.mergeSort.bind(SortAlgo);
    mergeSortStarted();
  }
}
